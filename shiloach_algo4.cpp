...
//#pragma omp parallel for
		for (int j = 0; j < temp.size(); j++)
		{
			...
			edge_times[tid] += std::chrono::duration_cast<std::chrono::nanoseconds>(se_end - se_start).count();
...
void createSummaryGraph(const EdgeList& edges)
{
	...
	{
		//set<pair<pair<int,int>,pair<int, int>>> temp = super_edges[i];
